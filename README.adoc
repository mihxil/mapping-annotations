...
All reflection stuff that in during profiling seemed to be expensive is implicitly cached. The result of json parsing is cached
only during one mapping call (thread locally actually), so that if may values are sourced from one json field (represented by a
`String` or `byte[]`), that field's value is parsed only once. That also significantly speed things up in some cases.
