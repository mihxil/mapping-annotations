...
Sometimes it's not feasible to map a suboject entirely by just adding annotaions.

E.g. a 'json' payload can be pointed to, but automatic unmapping of this is not feasible.

The mapper can have custom mappings then:

[source, java]
----
Mapper mapper = MAPPER.withCustomMapper(destinationType, (o) -> {
